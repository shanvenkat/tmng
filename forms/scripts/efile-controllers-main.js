...
efileControllers.controller('PrintCtrl', ['$scope', '$routeParams',   'RaaResource',
    function($scope, $routeParams, RaaResource) {         
        ...
        $scope.trademark.$promise.then(function(value) {
               ...
               // TM Literals
               angular.forEach($scope.trademark.tmLiterals, function(tmLiteral, index) {
                    $scope.literalElementTx = tmLiteral.literalElementTx;                    
              });
              ...
              angular.forEach($scope.trademark.tmInterestedParties, function(tmInterestedParty, index) {
                ...
                if (tmInterestedParty.fkTmIntrstdPartyRoleCd === 'CO') {
                    ...
                    $scope.COArray = COArray.join(', ');                  
                     ...
                     angular.forEach(tmInterestedParty.interestedParty.ipElectronicAddrs, function(ipElectronicAddr, index){                          
                             $scope.COAuthorizedEmailIn = ipElectronicAddr.authorizedEmailIn;                             

                             if (ipElectronicAddr.electronicAddress.electronicType === 'EMAIL'){
                                  $scope.COEmailAddress = ipElectronicAddr.electronicAddress.text
                             }                          
                     })
                ...
                // GST Terms
               angular.forEach($scope.trademark.tmClassGoodsServiceTerms, function(tmClassGoodsServiceTerm, index) {
                    $scope.fkFilingBasisCd = tmClassGoodsServiceTerm.fkFilingBasisCd;                    
              });
