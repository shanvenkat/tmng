...
efileServices.factory('DomesticRepResource', ['$resource',
    function($resource) {
        return $resource("/efile/rest/domesticrep/woa");
    }
]);

efileServices.factory('CorrespondenceResource', ['$resource',
    function($resource) {
        return $resource("/efile/rest/correspondence/woa");
    }
]);
...
efileServices.factory('MiscellaneousResourceStatementDocs', ['$resource',
                                                   function($resource) {
                                                       return $resource("/efile/rest/raa/miscstatementbyserial/:serialNum/:stTypeCd");
                                                   }
                                               ]);
...
efileApp.service('SignatureValidationgSevice', function() {
    var attorneyList = [];
    var tmCasesIdList = [];

    var valdiateSignature = function ($scope) {
        //validate signature.
        ...
        if($scope.signatureNameRequiredError || $scope.signatureRequiredError || $scope.electtonicSignWrongFormate){
    ...
    return {
        valdiateSignature : valdiateSignature
    }
});